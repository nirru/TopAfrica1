...
//        _day.setCloseAt(Utils.getTimeInMilli(hr,min,second));
    ...
    private void getStringAdress(double lat, double lng){
        FallbackReverseGeocodeObservable.createObservable(Locale.getDefault(),lat,lng,1)
                ...
                .subscribe(new DisplayAddressOnViewAction(country,text_state,city,text_district,text_neighbour,address,cordinate),new ErrorHandler(CreateBusiness.this));
    }
    @Override
    ...
    private void setUpSpinner(String businessType[]){
       SpineerView spineerView = new SpineerView(CreateBusiness.this, businessType,spinner, new SpineerView.OnSpinnerSelected() {
           @Override
...
//
        try {
            text_facebook_share.setText("http://www.facebook.com/");
            text_google_share.setText("http://www.google.com/");
            text_twitter_share.setText("http://www.twitter.com/");
            text_facebook_share.setSelection(text_facebook_share.getText().length());
            text_google_share.setSelection(text_google_share.getText().length());
            text_twitter_share.setSelection(text_twitter_share.getText().length());
...
////
        Observable<Boolean> description_observable = RxTextView.textChanges(text_business_description)
                .debounce(800, TimeUnit.MILLISECONDS)
                .observeOn(AndroidSchedulers.mainThread())
                .map(new Func1<CharSequence, Boolean>() {
                    @Override
                    public Boolean call(CharSequence s) {
                        if (s.length()>=1){
                            ...
                            Log.e("COUNTLLLL","" + count);
                            text_counter.setText(count+"/"+600);
                        }
                        return true;
                    }
                });
//
//
        _subscription = Observable.combineLatest(twitter_observable, description_observable, new Func2<Boolean, Boolean, Boolean>() {
            @Override
            public Boolean call(Boolean validPhone, Boolean validFax) {
                return validPhone && validFax;
            }
    ...
    private void uploadFiles(){
        if (logo_uri==null){
            logo_uri = Utils.getUriToResource(CreateBusiness.this,R.drawable.no_image_available);
        }
        if(pic_uri == null){
            pic_uri = Utils.getUriToResource(CreateBusiness.this,R.drawable.ic_pic_one);
        }
        if(pic_uri_2 == null){
            pic_uri_2 = Utils.getUriToResource(CreateBusiness.this,R.drawable.ic_pic_two);
        }
        if(pic_uri_3 == null){
            pic_uri_3 = Utils.getUriToResource(CreateBusiness.this,R.drawable.ic_pic_three);
        }
        ...
        if (currentSelectedItems.size() == 1){
            ...
            RxFirebaseDatabase.overWriteValue(ref,business_node_key,updateBusinessListForStepFour(as))
                    ...
                    .subscribe(new Subscriber<Firebase>() {
                        ...
                        public void onNext(Firebase firebase) {
                            showProgress(false);
                            AppController.getInstance().okAlert(CreateBusiness.this,"","Informations sent");
                        }
                    });
        }else if(currentSelectedItems.size() == 2){
            String as = currentSelectedItems.get(0);
