 ...
 */
public abstract class BaseSpashActivity extends AppCompatActivity implements
...
//       }else {
           if (checkPlayServices()) {
               if (!PermissionUtils.isLocationEnabled(BaseSpashActivity.this))
                   AppController.getInstance().showSettingsAlert(BaseSpashActivity.this);
               else {
...
//            toolbar.setNavigationIcon(R.drawable.vector_drawable_ic_menu_white___px);
    ...
    public void onConnected(@Nullable Bundle bundle) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (ActivityCompat.checkSelfPermission(BaseSpashActivity.this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                // TODO: Consider calling
                ActivityCompat.requestPermissions(BaseSpashActivity.this, new String[]{ACCESS_FINE_LOCATION}, LOCATION_PERMISSION_REQUEST_CODE);
                return;
            } else {
    ...
    public void onConnectionSuspended(int i) {
        if (i == CAUSE_SERVICE_DISCONNECTED) {
            Toast.makeText(BaseSpashActivity.this.getApplicationContext(), "Disconnected. Please re-connect.", Toast.LENGTH_SHORT).show();
        } else if (i == CAUSE_NETWORK_LOST) {
            Toast.makeText(BaseSpashActivity.this.getApplicationContext(), "Network lost. Please re-connect.", Toast.LENGTH_SHORT).show();
        }
        mGoogleApiClient.connect();
    }
    ...
    private void enableMyLocation() {
        if (ContextCompat.checkSelfPermission(BaseSpashActivity.this, Manifest.permission.ACCESS_FINE_LOCATION)
                != PackageManager.PERMISSION_GRANTED) {
            // Permission to access the location is missing.
            PermissionUtils.requestPermission(BaseSpashActivity.this, LOCATION_PERMISSION_REQUEST_CODE,
    ...
    public static class ErrorDialogFragment extends DialogFragment {
        ...
        public void onDismiss(DialogInterface dialog) {
            ((BaseSpashActivity) getActivity()).onDialogDismissed();
        }
    }
    ...
    protected boolean checkPlayServices() {
        int resultCode = GooglePlayServicesUtil
                .isGooglePlayServicesAvailable(BaseSpashActivity.this);
        if (resultCode != ConnectionResult.SUCCESS) {
            if (GooglePlayServicesUtil.isUserRecoverableError(resultCode)) {
                GooglePlayServicesUtil.getErrorDialog(resultCode, BaseSpashActivity.this,
                        PLAY_SERVICES_RESOLUTION_REQUEST).show();
            } else {
                Toast.makeText(BaseSpashActivity.this,
                        "This device is not supported.", Toast.LENGTH_LONG)
                        .show();
                ((AppCompatActivity) BaseSpashActivity.this).finish();
            }
            return false;
        }
        return true;
    ...
    public synchronized void buildGoogleApiClient() {
       gso = new GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
        ...
        mGoogleApiClient = new GoogleApiClient.Builder(BaseSpashActivity.this)
                .enableAutoManage(this, 0 /* clientId */, BaseSpashActivity.this)
                .addConnectionCallbacks(BaseSpashActivity.this)
                .addOnConnectionFailedListener(BaseSpashActivity.this)
